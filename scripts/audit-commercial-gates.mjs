...
assert.equal(defaultCorpus.includes(`aff=${expectedGamersGateId}`), true, "verified GamersGate tracking URL disappeared");
assert.equal(/gamersgate\.com[^>]+rel="[^"]*\bsponsored\b[^"]*"/i.test(defaultCorpus), true,
  "GamersGate sponsored relationship disappeared");
assert.equal(defaultCorpus.includes("affiliate_click"), true, "affiliate_click intent event disappeared");
